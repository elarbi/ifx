...
public class XMLInjector extends DefaultHandler implements IInjector {
	...
	/**
	 * SAX Locator, used to store elements locations in the target model for
	 * traceability.
	 */
	private Locator locator = null;

	/**
	 * Current element. Since it owns a reference to its parent, it behaves as a
	 * stack.
	 */
	...
	/**
	 * Injects an {@link InputStream} into an {@link EMFModel} (which conforms
	 * to the XML metamodel)
	 * 
	 * @param targetModel
	 *            the target {@link EMFModel} (which conforms to the XML
	 *            metamodel) to be injected
	 * @param source
	 *            the source {@link OutputStream} from which the target model
	 *            must be injected
	 * @param options
	...
	public void characters(char[] ch, int start, int length)
		...
		if (value.length() > 0) {
			EClass ec = (EClass) target.getReferenceModel()
					.getMetaElementByName("Text");
			EObject attr = ec.getEPackage().getEFactoryInstance().create(ec);
			attr.eSet(attr.eClass().getEStructuralFeature("name"), new String(
					"#text")); // as in DOM
			attr.eSet(attr.eClass().getEStructuralFeature("value"), new String(
					value));
			attr.eSet(attr.eClass().getEStructuralFeature("parent"), current);
			if (locator != null) {
				if (locator.getLineNumber() != -1) {
					int lineNumber = locator.getLineNumber();
					attr.eSet(attr.eClass().getEStructuralFeature("startLine"),
							new Integer(prevLineNumber));
					attr.eSet(attr.eClass().getEStructuralFeature("endLine"),
				...
				if (locator.getColumnNumber() != -1) {
					...
					attr.eSet(attr.eClass()
							.getEStructuralFeature("startColumn"), new Integer(
							prevColumnNumber));
					attr.eSet(attr.eClass().getEStructuralFeature("endColumn"),
	...
	public void startElement(String uri, String localName, String qName,
		...
		EClass ec;
		if (current == null) {
			ec = (EClass) target.getReferenceModel().getMetaElementByName(
					"Root");
		...
		EClass attrClass = (EClass) target.getReferenceModel()
				.getMetaElementByName("Attribute");
		for (int i = 0; i < attributes.getLength(); i++) {
			EObject attr = attrClass.getEPackage().getEFactoryInstance()
					.create(attrClass);
			attr.eSet(attrClass.getEStructuralFeature("name"), new String(
					attributes.getQName(i)));
			attr.eSet(attrClass.getEStructuralFeature("value"), new String(
		...
		if (locator != null) {
			if (locator.getLineNumber() != -1) {
				current.eSet(ec.getEStructuralFeature("startLine"),
						new Integer(prevLineNumber));
				prevLineNumber = locator.getLineNumber();
			}
			if (locator.getColumnNumber() != -1) {
				current.eSet(ec.getEStructuralFeature("startColumn"),
						new Integer(prevColumnNumber));
	...
	public void endElement(String uri, String localName, String qName)
			throws SAXException {
		EObject parent = (EObject) current.eGet(current.eClass()
				.getEStructuralFeature("parent"));
		if (locator != null) {
			if (locator.getLineNumber() != -1) {
				prevLineNumber = locator.getLineNumber();
				current.eSet(current.eClass().getEStructuralFeature("endLine"),
						new Integer(prevLineNumber));
			}
			if (locator.getColumnNumber() != -1) {
				prevColumnNumber = locator.getColumnNumber();
				current.eSet(current.eClass()
						.getEStructuralFeature("endColumn"), new Integer(
						prevColumnNumber));
			}
		}
