...
public class XMLExtractor implements IExtractor {
	 ...
	 * {@inheritDoc}
	 * 
	 * @see org.eclipse.m2m.atl.core.IExtractor#extract(org.eclipse.m2m.atl.core.IModel,
	 *      java.lang.String)
	 */
	 ...
	 * <li><b>Workspace Resource: </b>
	 * <code>platform:/resource/<i>path</i></code></li>
	 * </ul>
	 * 
	 * @see org.eclipse.m2m.atl.core.IExtractor#extract(org.eclipse.m2m.atl.core.IModel,
	...
	/**
	 * Extracts an {@link EMFModel} (which conforms to the XML metamodel) to an
	 * {@link OutputStream}.
	 * 
	 * @param sourceModel
	 *            the source {@link EMFModel} (which conforms to the XML
	 *            metamodel) to be extracted
	 * @param target
	 *            the target {@link OutputStream} where the source model must be
	 *            extracted
	 * @param options
	 *            the extraction parameters (none are required there)
	 */
	public void extract(IModel sourceModel, OutputStream target,
			Map<String, Object> options) throws ATLCoreException {
		...
		serializeContent(rootElements.iterator().next(), xmlModel, outStream,
				"");
	...
	/**
	 * The recursive method effectively serializing the content of the XML
	 * model.
	 * 
	 * @param xmlModelElement
	 *            the current XML model element ({@link EObject}) to be
	 *            serialized
	 * @param xmlModel
	 *            the XML model ({@link EMFModel}) to be serialized
	 * @param outStream
	 *            the target {@link PrintStream} where the source model must be
	 *            extracted
	 * @param indent
	 *            the current indentation level
	 */
	@SuppressWarnings("unchecked")
	private void serializeContent(EObject xmlModelElement, EMFModel xmlModel,
		...
		if (oTypeName.equals("Element") || oTypeName.equals("Root")) {
			...
			EList<EObject> children = (EList<EObject>) xmlModelElement
					.eGet(xmlModelElement.eClass().getEStructuralFeature(
							"children"));
			boolean hasElements = false;
			boolean hasTexts = false;
			for (Iterator<EObject> i = children.iterator(); i.hasNext();) {
				EObject currentModelElement = i.next();
				String typeName = getStringValue(currentModelElement.eClass(),
						"name");
				if (typeName.equals("Attribute")) {
					outStream
							.print(" "
									+ getStringValue(currentModelElement,
											"name")
									+ "=\""
									+ convertText(
											getStringValue(currentModelElement,
			...
			if (hasElements) {
				...
				for (Iterator<EObject> i = children.iterator(); i.hasNext();) {
					...
					String typeName = getStringValue(
							currentModelElement.eClass(), "name");
					if (typeName.equals("Element")) {
						this.serializeContent(currentModelElement, xmlModel,
								outStream, indent + "  ");
					} else if (typeName.equals("Text")) {
						outStream.print(convertText(
								getStringValue(currentModelElement, "value"),
								false));
					}
				}
	...
	/**
	 * Returns the string value of a specified structural feature (from a given
	 * model element).
	 * 
	 * @param modelElement
	 ...
	 * @return
	 */
	private String getStringValue(EObject modelElement,
			String structuralFeatureName) {
		Object ret = modelElement.eGet(modelElement.eClass()
				.getEStructuralFeature(structuralFeatureName));

		if (!(ret instanceof String))
			throw new RuntimeException("could not read "
					+ structuralFeatureName + " of " + modelElement + " : "
					+ modelElement.eClass());
	...
	/**
	 * Converts the input string into an XML compatible string format (special
	 * character's conversion).
	 * 
	 * @param in
	 * @param inAttr
	 * @return
	...
	private String convertText(String in, boolean inAttr) {
		String ret = in.replaceAll("&", "&amp;").replaceAll("<", "&lt;")
				.replaceAll(">", "&gt;");
