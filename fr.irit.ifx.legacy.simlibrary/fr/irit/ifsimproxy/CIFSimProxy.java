...
public class CIFSimProxy  {
    ...
    public CIFSimProxy() {
		...
		try {
	    	...
	    	DOMImplementationLS impl = //(DOMImplementationLS)_di.getFeature("LS", "3.0");
    ...
    /** proxy commands
     */
    public void Connect(String host, int port) throws UnknownHostException, IOException, SecurityException, EIFSimException {
    ...
    public void Undo() throws EIFSimException {
        if(scenarioCurrent <= 0) return;
        _SendXMLRequest("<IFSUndo/>");
        testOK();
        
        scenarioCurrent--;
        cacheConfig = cacheTransitions = null;
        if(scenarioNextStep == null) scenarioNextStep = scenario.getDocumentElement().getLastChild();
        else scenarioNextStep = scenarioNextStep.getPreviousSibling();
    ...
    public void Redo() throws EIFSimException {
        if(scenarioCurrent >= scenarioMax) return;
    ...
    public void GotoStep(int step) throws EIFSimException {
        if(step < 0 || step > scenarioMax || step == scenarioCurrent) return;
        _playInterrupt = false;
        _playLength = scenarioMax;
        _playStep = scenarioCurrent;
    ...
    public void Fire(int trno) throws EIFSimException {
        ...
        while(scenarioMax > scenarioCurrent) { // remove elements from current+1 to max
            s.removeChild(s.getLastChild());
            scenarioMax--;
        }
        // add element
        Node trans = cacheTransitions.getFirstChild();
        while(trno-- > 0) trans = trans.getNextSibling();
    ...
    public void Play(File saved) throws EIFSimException {
        ...
        try {
            ...
            throw new EIFSimException("Cannot parse file " + saved.toString() + ": " + ex1.toString());
        }
    }
    ...
    public int getPlayStep() { return _playStep; }
    public int getPlayLength() { return _playLength; }
    public void interruptPlay() { _playInterrupt = true; }
    
    public void Play(Element saved) throws EIFSimException {
        _playLength = saved.getChildNodes().getLength();
        _playStep = 0;
        _playInterrupt = false;
        for(Node child = saved.getFirstChild(); child != null && !_playInterrupt; child = child.getNextSibling() ) {
            ...
            try {
                ...
                _playLength = 0;
                throw new EIFSimException("Scenario step " + _playStep + ": " + ex.getMessage());
            }
            Fire(index);
        }
        _playLength = 0;
    ...
    public int getTransitionNo(Node xml) throws EIFSimException {
        ...
        for(; tr != null; tr = tr.getNextSibling(), index++ ) {
            ...
            if(modelTrans.equals(scnTrans)) {
                if(chosenindex != -1)
                    throw new EIFSimException("non-determinism cannot be resolved between two identical transitions (" + chosenindex + " and " + index + ")");
                chosenindex = index;
            }
        }
        
        if(chosenindex==-1)
            throw new EIFSimException("XML node does not represent a fireable transition in the current state.");
        
        return chosenindex;
    }
    
    public int getScenarioNextStepTransitionNo() {
        if(scenarioNextStep==null) return -1;
        int x;
        try {
    ...
    protected void _SendXMLRequest(String xml) throws EIFSimException {
        ...
        while(nbHeadSpaces-- > 0)socket_out.print(" ");
        socket_out.print(PDUHead); 
        socket_out.print(XMLHeader);
        socket_out.print(xml); socket_out.flush();
    }
    ...
    protected void _initDocumentBuilder() throws EIFSimException {
        try {
            if(_dbf == null) _dbf = DocumentBuilderFactory.newInstance();
            if(_db == null) {
    ...
    protected void _initScenario() throws EIFSimException {
        ...
        //_di = (DOMImplementation)com.sun.org.apache.xerces.internal.dom.DOMImplementationImpl.getDOMImplementation();
        scenario = _di.createDocument("http://www-verimag.imag.fr", "scenario", null);
    }
    
    public void writeScenario(File file) throws java.io.FileNotFoundException, java.io.IOException {
        if(_di.getClass().getName().equals("org.apache.xerces.dom.DOMImplementationImpl")) {
...
/*    public void notifyViews() {
        ...
        if(scenario!=null)System.out.println("%%%"+scenario.getDocumentElement());
        System.out.println("______");
        System.out.println("%%%"+scenarioNextStep);
        super.notifyViews();
    }*/
     ...
     */
    public static void main(String[] args) throws UnknownHostException, IOException, SecurityException, EIFSimException {
