 ...
 * </copyright>
 *
 * $Id: OCLResource.java,v 1.1 2010/03/11 10:13:26 ewillink Exp $
 */
...
public class OCLResource extends XMIResourceImpl {
	...
	/**
	 * Loads an OCL expression from the specified <code>path</code>. The OCL
	 * expression is converted to a string using a custom AST visitor that
	 * renders the string representation.
	 * 
	 * @param path
	 *            the absolute path of the XMI file to load
	 * @return the string representation of the OCL expression, if found
	 * @throws IOException
	 *             if anything goes wrong in loading the XMI file
	 */
	 ...
	 * 
	 * @param path
	 *            the fully-qualified path of the XMI file to save
	 * @param document
	 ...
	 * @throws IOException
	 *             if anything goes wrong in saving
	...
	public static void save(String path, OCLDocument document, String expression)
		...
		OCL<?, Object, ?, ?, ?, ?, ?, ?, ?, Object, ?, ?> ocl = document
				.getOCLFactory().createOCL(document.getModelingLevel(), res);
		OCLHelper<Object, ?, ?, Object> helper = ocl.createOCLHelper();

		// set our helper's context classifier to parse against it
		ConstraintKind kind = document.getModelingLevel().setContext(helper,
				document.getOCLContext(), document.getOCLFactory());
		...
		case M1:
			...
			parsed = ocl.getEnvironment().getUMLReflection()
					.getSpecification(constraint).getBodyExpression();
			break;
		}
